...
export function KanbanBoardView(props: { board: Model<Board> }) {
  const [board, dispatch] = useReducer(boardReducer, null, () => props.board);
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      dispatch({ type: "edit_board_title", title: title });
    }, 1000);
      ...
      <div className="p-6">
        <div className="flex flex-row items-center">
          <input
            ...
            className="text-4xl font-medium w-full rounded-lg p-2 my-2 bg-transparent flex-1 focus:bg-gray-50 hover:bg-gray-50 focus:dark:bg-gray-800 hover:dark:bg-gray-800"
          />
        ...
        <Modal open={newStatus} closeCB={() => setNewStatus(false)}>
          <CreateStatus boardId={props.board.id!} />
        </Modal>
      </div>
    </DashboardBase>
  );
